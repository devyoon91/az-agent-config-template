...
```
az-agent-config/
├── agents/                     # 도메인 특화 에이전트 프로필 (Profile)
│   ├── reviewer/               # 코드 리뷰 전문가
│   │   ├── agent.yaml
│   │   └── prompts/
...
Agent 0 (마스터, Developer)
    ...
    │
    │── Skill (지침 자동 로드) ──────────────────
    └── trigger_patterns 매칭 시 SKILL.md 로드 → 절차/체크리스트 실행
```
...
|---|---|---|
| 정체 | 독립된 전문가 에이전트 | 지침서/매뉴얼 |
| 실행 | 별도 에이전트 인스턴스 생성 | 현재 에이전트에 지침 추가 |
| 활성화 | `call_subordinate` 명시 호출 | `trigger_patterns` 자동 매칭 |
...
`skills/`에 SKILL.md 기반 지침서를 넣으면 Agent Zero가 대화 맥락에서 **자동으로 매칭하여 로드**합니다. Profile(서브 에이전트)과 달리 별도 에이전트를 생성하지 않고, 현재 에이전트에 절차/지침을 추가하는 가벼운 방식입니다.
...
```markdown
---
name: ecs-domain-dict
description: ECS Telecom 도메인 용어 사전 검색 및 해설
version: 1.0.0
tags: [ecs, glossary, terminology]
trigger_patterns:
  - "용어"
  - "뭐야"
  - "약어"
...
- 별도의 전문가 인격이 필요하면 → `agents/` (Profile)
- 절차/체크리스트/지침만 있으면 되면 → `skills/` (Skill)
- `trigger_patterns`에 핵심 키워드를 넣으면 대화 중 자동 활성화
- 스킬 안에 `scripts/`, `templates/`, `docs/` 하위 디렉토리를 둘 수 있음
